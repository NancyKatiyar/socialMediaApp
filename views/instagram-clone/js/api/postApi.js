...
//           if (post.visibility === 'Only Me' && post.user_id === user_id) return true;
//           return false;
//         });
...
//       const bodyData = { comment_description }; // Only add parent_id if it's a reply
...
//   try {
//     const res = await fetch(`http://localhost:3000/getAllComments`, {
//       method: "GET",
//     });
//     console.log(res)
...
//     document.querySelectorAll('[id^="comments-"]').forEach((div) => (div.innerHTML = ""));
//     const comments = data.data.comments;

//     comments.forEach((comment) => {
//       const commentsContainer = document.getElementById(`comments-${comment.post_id}`);
//       if (!commentsContainer) return;

//     const commentHTML = `
//   <div class="post__single-comment" data-comment-id="${comment.id}">
//     <b>${comment.User?.user_name || "User"}:</b> ${comment.comment_description}
//     <button class="reply-btn" style="margin-left:10px; font-size:12px; cursor:pointer;">Reply</button>
//   </div>
// `;
...
//         addReplies(comment.replies, commentsContainer);
//       }
//     });
//   } catch (err) {
...
// function addReplies(replies, container) {
//   replies.forEach((reply) => {
//   const replyHTML = `
//   <div class="post__single-comment reply" data-comment-id="${reply.id}" style="margin-left:20px;">
//     <b>${reply.User?.user_name || "User"}:</b> ${reply.comment_description}
//     <button class="reply-btn" style="margin-left:10px; font-size:12px; cursor:pointer;">Reply</button>
//   </div>
// `;
...
//       addReplies(reply.replies, container);
//     }
//   });
// }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Render posts (unchanged logic, but use data-post-id)
  async function loadPosts() {
    const token = document.cookie.split("; ").find((r) => r.startsWith("token="))?.split("=")[1];
    let user_id;
    if (token) {
      try { user_id = JSON.parse(atob(token.split(".")[1])).id; } catch(e){/* ignore */ }
    }
      ...
      posts.forEach(post => {
        const postHTML = `
          <article class="post" data-post-id="${post.id}">
            <div class="post__header">
              <div class="post__profile">
                <a href="#" class="post__avatar"><img src="https://images.unsplash.com/photo-1761839257349-037aea1d94de?ixlib=rb-4.1.0&auto=format&fit=crop&q=80&w=1169" /></a>
                <a href="#" class="post__user">${post.User?.user_name || ""}</a>
              </div>
            </div>
            ...
            <div class="post__footer">
              ...
              <div class="post__infos">
                <div class="post__description"><span>${post.post_title}</span></div>
                <div class="post__comments" id="comments-${post.id}"></div>
                <span class="post__date-time">${new Date(post.createdAt).toLocaleString()}</span>
              </div>
            </div>
          </article>
        `;
  ...
  // --- Fetch comments for a specific post and render them
  async function getCommentsForPost(postId) {
    if (!postId) return;
    try {
      const res = await fetch(`http://localhost:3000/getAllReplies/${postId}`, { method: "GET" });
      const data = await res.json();
      console.log("API Response:", data);
      if (!data.success) return;

      const commentsContainerElement = document.getElementById(`comments-${postId}`);
      if (!commentsContainerElement) return;

      // clear only for this post
      commentsContainerElement.innerHTML = "";

      const comments = data.data?.comments || [];
      // render top-level comments (should be only parent_id === null from your API)
      comments.forEach(comment => {
        renderComment(comment, commentsContainerElement);
      });
  ...
  function renderComment(comment, container) {
    ...
    html.className = "post__single-comment";
    html.dataset.commentId = comment.id;            // becomes commentDiv.dataset.commentId
    html.innerHTML = `
      <b>${comment.User?.user_name || "User"}:</b> ${escapeHtml(comment.comment_description)}
      <button class="reply-btn" style="margin-left:10px;font-size:12px;cursor:pointer;">Reply</button>
      <div class="replies-container" style="margin-left:16px;"></div>
    `;
    container.appendChild(html);

    // render replies recursively inside .replies-container
    const repliesContainer = html.querySelector(".replies-container");
    if (comment.replies && comment.replies.length) {
      comment.replies.forEach(r => {
        renderReplyRecursive(r, repliesContainer);
      });
    }
  }

function renderReplyRecursive(reply, container) {
  const div = document.createElement("div");
  div.className = "post__single-comment reply";
  div.dataset.commentId = reply.id;
  div.style.marginLeft = "20px";

  div.innerHTML = `
    <b>${reply.User?.user_name || "User"}:</b> ${escapeHtml(reply.comment_description)}
    <button class="reply-btn reply-small-btn">Reply</button>
    <div class="replies-container" style="margin-left:12px;"></div>
  `;

  container.appendChild(div);

  if (reply.replies && reply.replies.length) {
    const inner = div.querySelector(".replies-container");
    reply.replies.forEach(r => renderReplyRecursive(r, inner));
  }
}


  // small escape helper to avoid accidental HTML injection
  ...
  // --- Event delegation: clicks inside postsContainer
  postsContainer.addEventListener("click", (e) => {
    const commentBtn = e.target.closest(".comment-btn");
    const replyBtn = e.target.closest(".reply-btn");
    ...
    if (commentBtn) {
      const postArticle = e.target.closest(".post");
      currentPostId = Number(postArticle.dataset.postId); // data-post-id -> dataset.postId
      currentParentId = null;
      console.log("Open comments for post:", currentPostId);

      // open modal and load comments for this post
      if (commentModal) commentModal.style.display = "block";
      if (commentText) commentText.value = "";
      getCommentsForPost(currentPostId);
      return;
    }

    if (replyBtn) {
      // set currentParentId to the comment that owns this reply button
      const commentDiv = e.target.closest(".post__single-comment");
      if (!commentDiv) return;
      currentParentId = Number(commentDiv.dataset.commentId);
      // find the post ancestor to set currentPostId
      const postArticle = e.target.closest(".post");
      currentPostId = Number(postArticle.dataset.postId);
      console.log("Replying to comment:", currentParentId, "on post:", currentPostId);
